...
func (j *Jar) mergeFrom(r io.Reader) error {
	bytesJSON, _ := ioutil.ReadAll(r)
	if j.options.EncryptionPassword != "" {
		j.crypt.Crypt(bytesJSON)
	}
	decoder := json.NewDecoder(bytes.NewReader(bytesJSON))
	// Cope 1with old cookiejar format by just discarding
